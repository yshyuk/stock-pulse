...
### 미해결
- **삼성전자·SK하이닉스 두 종목만 담긴 메시지가 계속 온다는 사용자 제보.**
  현재 배치는 아님이 확인됨(리포트에 `데이터 출처: krx-all`, 더미 배너 없음,
  `NAVER_ENABLED=false`). 다른 프로세스/스케줄러가 있는지 미확인.
  `NAVER_SYMBOLS`가 `005930,000660`이라 네이버 경로가 의심되나 꺼져 있다

### 다음에 볼 것 (우선순위)
1. 위 미해결 건 원인 규명
2. **스크리닝 상한 30 → 50** — 상한 30은 API 종량제 비용 때문이었는데 구독 전환으로
   제약이 사라졌다. 분석이 "474종목 매칭 중 30개만 표시, ±5~15% 구간은 안 보인다"고 지적.
   env 한 줄(`STOCKPULSE_SCREENING_MAX`)
3. self-hosted 러너 등록 (현재 0대, jar 수동 배포 중)
4. 임계값 조정 — 실데이터 2~4주 누적 후
5. `SnapshotService` 히스토리 조회 — 현재 종목별. 누적 후 재측정 필요
   (2,765 × 260 ≈ 72만 행/실행)
6. `BatchPipeline` 협력자 16개 — 분리 검토
