...
분석 결과는 텔레그램과 디스코드 **양쪽**으로 갑니다(설정된 채널만).
텔레그램은 `parse_mode=HTML`로 보내 `##` 헤더와 `**굵게**`가 실제로 굵게 표시됩니다 —
MarkdownV2는 이스케이프할 문자가 많아 한 글자만 놓쳐도 400이 나므로 쓰지 않습니다.
분할은 줄 경계에서만 합니다(태그 중간을 자르면 깨진 HTML이 됩니다).
