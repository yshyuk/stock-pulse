...
| P2-5 | 다운타임 체결이 일손실 가드에 미반영 | 리컨실이 phantom 포지션을 `realizedPnl=null`로 청산 → 엔진 다운 중 실현된 손실이 일손실 한도 계산에서 0으로 집계 | KIS 체결내역 조회로 실현손익 소급, 또는 보수적 추정 |
| P2-6 | broker-only 미관리 포지션 | 리컨실이 목표/손절 없는 계좌 보유를 감지만 하고 청산 안 함 → 무한 노출 | `flatten-unknown-holdings` 정책/설정 도입 |
